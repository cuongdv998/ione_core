...
public class ResObjectItemType : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    [Required]
    public virtual ResObjectItemTypeStatus Status { get; private set; }
    ...
    protected ResObjectItemType()
    {
        // For ORM
    }

    public ResObjectItemType(Guid id, string code, string name, string? description, ResObjectItemTypeStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResObjectItemTypeStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResObjectItemTypeStatus status)
    {
        SetStatus(status);
    }
}
