...
public class ResClaimType : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(25)]
    ...
    [Required]
    public virtual ResClaimTypeStatus Status { get; private set; }

    protected ResClaimType()
    {
        // For ORM
    }

    public ResClaimType(Guid id, string code, string name, ResClaimTypeStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResClaimTypeStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResClaimTypeStatus status)
    {
        SetStatus(status);
    }
}
