...
[Table("pro_rule_type")]
public class ProRuleType : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    [Required]
    public virtual ProRuleTypeStatus Status { get; private set; }

    protected ProRuleType()
    {
        // For ORM
    }

    public ProRuleType(Guid id, string code, string name, string? description, ProRuleTypeStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ProRuleTypeStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ProRuleTypeStatus status)
    {
        SetStatus(status);
    }
}
