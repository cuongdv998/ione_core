...
[Table("res_event")]
public class ResEvent : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    [Required]
    public virtual ResEventStatus Status { get; private set; }

    // Navigation property
    public virtual ICollection<ResEventNotifyTemplate> NotifyTemplates { get; protected set; }

    protected ResEvent()
    {
        // For ORM
        NotifyTemplates = new List<ResEventNotifyTemplate>();
    }

    public ResEvent(Guid id, string code, string name, string? description, ResEventStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResEventStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResEventStatus status)
    {
        SetStatus(status);
    }
}
