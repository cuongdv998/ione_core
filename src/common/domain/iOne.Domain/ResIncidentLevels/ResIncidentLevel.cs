...
public class ResIncidentLevel : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(25)]
    ...
    [Required]
    public virtual ResIncidentLevelStatus Status { get; private set; }

    protected ResIncidentLevel()
    {
        // For ORM
    }

    public ResIncidentLevel(Guid id, string code, string name, ResIncidentLevelStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResIncidentLevelStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResIncidentLevelStatus status)
    {
        SetStatus(status);
    }
}
