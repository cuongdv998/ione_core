...
[Table("res_car_type")]
public class ResCarType : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    [Required]
    public virtual ResCarTypeStatus Status { get; private set; }

    protected ResCarType()
    {
        // For ORM
    }

    public ResCarType(Guid id, string code, string name, string? description, ResCarTypeStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResCarTypeStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResCarTypeStatus status)
    {
        SetStatus(status);
    }
}
