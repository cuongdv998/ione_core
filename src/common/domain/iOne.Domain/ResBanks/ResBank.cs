...
[Table("res_bank")]
public class ResBank : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(25)]
    ...
    [Required]
    public virtual ResBankStatus Status { get; private set; }
    ...
    public ResBank(Guid id, string code, string name, ResBankStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResBankStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResBankStatus status)
    {
        SetStatus(status);
    }
}
