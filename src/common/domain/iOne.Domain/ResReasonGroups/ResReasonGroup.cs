...
public class ResReasonGroup : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    public virtual ICollection<ResReason> Reasons { get; private set; } = new List<ResReason>();

    protected ResReasonGroup()
    {
        // For ORM
    }

    public ResReasonGroup(Guid id, string code, string name, string? description, ResReasonGroupStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
