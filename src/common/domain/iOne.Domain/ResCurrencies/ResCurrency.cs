...
[Table("res_currency")]
public class ResCurrency : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    [Required]
    public virtual ResCurrencyStatus Status { get; private set; }
    ...
    public ResCurrency(Guid id, string code, string name, string? description, ResCurrencyStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResCurrencyStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResCurrencyStatus status)
    {
        SetStatus(status);
    }
}
