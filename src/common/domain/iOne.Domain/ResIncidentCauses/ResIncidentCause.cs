...
public class ResIncidentCause : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(25)]
    ...
    [Required]
    public virtual ResIncidentCauseStatus Status { get; private set; }

    protected ResIncidentCause()
    {
        // For ORM
    }

    public ResIncidentCause(Guid id, string code, string name, ResIncidentCauseStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResIncidentCauseStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResIncidentCauseStatus status)
    {
        SetStatus(status);
    }
}
