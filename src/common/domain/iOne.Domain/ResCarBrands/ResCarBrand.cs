...
[Table("res_car_brand")]
public class ResCarBrand : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    public virtual ICollection<ResCarModel> CarModels { get; private set; } = new List<ResCarModel>();
    ...
    protected ResCarBrand()
    {
        // For ORM
    }

    public ResCarBrand(Guid id, string code, string name, string? description, ResCarBrandStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
