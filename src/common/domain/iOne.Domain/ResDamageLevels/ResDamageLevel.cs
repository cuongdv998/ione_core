...
public class ResDamageLevel : FullAuditedAggregateRoot<Guid>
{
    [Required]
    ...
    [Required]
    public virtual ResDamageLevelStatus Status { get; private set; }

    // Navigation property for many-to-one relationship
    public virtual ResObjectType ObjectType { get; private set; } = null!;

    protected ResDamageLevel()
    {
        // For ORM
    }

    public ResDamageLevel(Guid id, Guid objectTypeId, string code, string name, string? description, ResDamageLevelStatus status)
        : base(id)
    {
    ...
    private void SetStatus(ResDamageLevelStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResDamageLevelStatus status)
    {
        SetStatus(status);
    }
}
