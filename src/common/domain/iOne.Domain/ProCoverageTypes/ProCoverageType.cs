...
public class ProCoverageType : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    [Required]
    public virtual ProCoverageTypeStatus Status { get; private set; }

    protected ProCoverageType()
    {
        // For ORM
    }

    public ProCoverageType(Guid id, string code, string name, ProCoverageTypeStatus status, string? description = null)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ProCoverageTypeStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ProCoverageTypeStatus status)
    {
        SetStatus(status);
    }
