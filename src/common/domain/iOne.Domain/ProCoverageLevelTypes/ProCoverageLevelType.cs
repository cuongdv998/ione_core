...
public class ProCoverageLevelType : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    [Required]
    public virtual ProCoverageLevelTypeStatus Status { get; private set; }

    protected ProCoverageLevelType()
    {
        // For ORM
    }

    public ProCoverageLevelType(Guid id, string code, string name, string? description, ProCoverageLevelTypeStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ProCoverageLevelTypeStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ProCoverageLevelTypeStatus status)
    {
        SetStatus(status);
    }
}
