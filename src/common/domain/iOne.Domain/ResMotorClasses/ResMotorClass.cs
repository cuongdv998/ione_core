...
public class ResMotorClass : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    [Required]
    public virtual ResMotorClassStatus Status { get; private set; }

    protected ResMotorClass()
    {
        // For ORM
    }

    public ResMotorClass(Guid id, string code, string name, string? description, ResMotorClassStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResMotorClassStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResMotorClassStatus status)
    {
        SetStatus(status);
    }
}
