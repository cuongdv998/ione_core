...
[Table("res_car_line")]
public class ResCarLine : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    [Required]
    public virtual ResCarLineStatus Status { get; private set; }

    // Navigation property for one-to-many relationship
    public virtual ICollection<ResCarGroup> CarGroups { get; private set; } = new List<ResCarGroup>();

    protected ResCarLine()
    {
        // For ORM
    }

    public ResCarLine(Guid id, string code, string name, string? description, ResCarLineStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResCarLineStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResCarLineStatus status)
    {
        SetStatus(status);
    }
}
