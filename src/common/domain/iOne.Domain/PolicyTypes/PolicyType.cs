...
[Table("policy_type")]
public class PolicyType : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(50)]
    ...
    [Required]
    public virtual PolicyTypeStatus Status { get; private set; }
    ...
    public PolicyType(Guid id, string code, string name, string? description, PolicyTypeStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(PolicyTypeStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(PolicyTypeStatus status)
    {
        SetStatus(status);
    }
}
