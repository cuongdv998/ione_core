...
public class ResPaymentMethod : FullAuditedAggregateRoot<Guid>
{
    [Required]
    [MaxLength(25)]
    ...
    [Required]
    public virtual ResPaymentMethodStatus Status { get; private set; }

    protected ResPaymentMethod()
    {
        // For ORM
    }

    public ResPaymentMethod(Guid id, string code, string name, ResPaymentMethodStatus status)
        : base(id)
    {
        SetCode(code);
        SetName(name);
    ...
    private void SetStatus(ResPaymentMethodStatus status)
    {
        Status = status;
    }
    ...
    public virtual void UpdateStatus(ResPaymentMethodStatus status)
    {
        SetStatus(status);
    }
}
