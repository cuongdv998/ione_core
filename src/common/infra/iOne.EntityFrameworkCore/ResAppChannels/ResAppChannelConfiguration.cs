...
namespace iOne.EntityFrameworkCore.ResAppChannels;

public class ResAppChannelConfiguration : IEntityTypeConfiguration<ResAppChannel>
{
    public void Configure(EntityTypeBuilder<ResAppChannel> builder)
    {
        // ✅ Table name: snake_case (PostgreSQL convention)
        builder.ToTable("res_app_channel", t =>
        {
        ...
        builder.Property(x => x.Status)
            ...
            .HasConversion<string>(
                ...
                v => Enum.Parse<ResAppChannelStatus>(v, true) // Parse "active" → Active, "deactive" → Deactive
            )
            .HasMaxLength(10)
            .IsRequired()
        ...
        builder.Property(x => x.Type)
            .HasColumnName("type")
            .HasConversion<string>(
                v => v != null ? v.Value.ToString().ToLowerInvariant() : null!, // Enum → "email"
                v => !string.IsNullOrWhiteSpace(v) ? (ResAppChannelType)Enum.Parse(typeof(ResAppChannelType), v, true) : (ResAppChannelType?)null // "email" → Enum
            )
            .HasMaxLength(20);
