using System;
using System.Linq;
using System.Threading.Tasks;
using iOne.Policy;
using iOne.Policy.Localization;
using iOne.Policy.Permissions;
using iOne.Policy.PolicyTypes;
using iOne.PolicyTypes; // For Entity, Manager, Repository
using Microsoft.AspNetCore.Authorization;
using Microsoft.EntityFrameworkCore;
using Volo.Abp;
using Volo.Abp.Application.Dtos;
using Volo.Abp.Application.Services;
using Volo.Abp.Domain.Repositories;

namespace iOne.Policy.PolicyTypes;

[RemoteService(Name = PolicyRemoteServiceConsts.RemoteServiceName)]
[Authorize(PolicyTypePermissions.Default)]
public class PolicyTypeAppService : CrudAppService<
    PolicyType,
    PolicyTypeDto,
    Guid,
    GetPolicyTypesInput,
    CreatePolicyTypeDto,
    UpdatePolicyTypeDto>, IPolicyTypeAppService
{
    protected PolicyTypeManager Manager { get; }
    protected IPolicyTypeRepository PolicyTypeRepository { get; }

    public PolicyTypeAppService(
        IRepository<PolicyType, Guid> repository,
        PolicyTypeManager manager,
        IPolicyTypeRepository policyTypeRepository)
        : base(repository)
    {
        Manager = manager;
        PolicyTypeRepository = policyTypeRepository;
        LocalizationResource = typeof(PolicyResource);
        GetPolicyName = PolicyTypePermissions.View;
        GetListPolicyName = PolicyTypePermissions.View;
        CreatePolicyName = PolicyTypePermissions.Create;
        UpdatePolicyName = PolicyTypePermissions.Edit;
        DeletePolicyName = PolicyTypePermissions.Delete;
    }

    public override async Task<PolicyTypeDto> CreateAsync(CreatePolicyTypeDto input)
    {
        // Validate duplicate code
        var normalizedCode = input.Code?.Trim().ToUpperInvariant();
        if (!string.IsNullOrWhiteSpace(normalizedCode))
        {
            if (await PolicyTypeRepository.IsCodeExistsAsync(normalizedCode))
            {
                throw new UserFriendlyException(
                    L["PolicyType:CodeExists"].Value.Replace("{Code}", normalizedCode)
                );
                //throw new BusinessException("Policy:PolicyType:CodeExists")
                //    .WithData("Code", normalizedCode);
            }
        }

        var entity = new PolicyType(
            GuidGenerator.Create(),
            input.Code,
            input.Name,
            input.Description,
            input.Status
        );

        await Manager.CreateAsync(entity);

        return ObjectMapper.Map<PolicyType, PolicyTypeDto>(entity);
    }

    public override async Task<PolicyTypeDto> UpdateAsync(Guid id, UpdatePolicyTypeDto input)
    {
        // Load entity with tracking to ensure EF Core can detect changes
        // Repository.GetAsync will throw exception if not found, never returns null
        var entity = await Repository.GetAsync(id);

        // ⚠️ QUAN TRỌNG: Chỉ update Name, Description và Status, không update Code
        // Code is immutable, so no duplicate validation needed
        await Manager.UpdateAsync(entity, input.Name, input.Description, input.Status);

        // Ensure changes are saved (Repository.UpdateAsync should handle this, but explicitly save to ensure audit logging)
        await CurrentUnitOfWork.SaveChangesAsync();

        return ObjectMapper.Map<PolicyType, PolicyTypeDto>(entity!);
    }

    public override async Task DeleteAsync(Guid id)
    {
        // Repository.GetAsync will throw exception if not found, never returns null
        var entity = await Repository.GetAsync(id);
        
        // Delete to trigger audit logging with ChangeType = Deleted
        // FullAuditedAggregateRoot uses soft delete, so entity is not actually removed from database
        await Repository.DeleteAsync(entity);
        await CurrentUnitOfWork.SaveChangesAsync();
        
        // Update status to Deactive (entity still exists in database due to soft delete)
        entity!.UpdateStatus(PolicyTypeStatus.Deactive);
        await Repository.UpdateAsync(entity);
        await CurrentUnitOfWork.SaveChangesAsync();
    }

    protected override async Task<IQueryable<PolicyType>> CreateFilteredQueryAsync(GetPolicyTypesInput input)
    {
        var query = await ReadOnlyRepository.GetQueryableAsync();

        // Filter by Code (case-insensitive, contains)
        if (!string.IsNullOrWhiteSpace(input.Code))
        {
            query = query.Where(x => EF.Functions.ILike(x.Code, $"%{input.Code}%"));
        }

        // Filter by Name (case-insensitive, contains)
        if (!string.IsNullOrWhiteSpace(input.Name))
        {
            query = query.Where(x => EF.Functions.ILike(x.Name, $"%{input.Name}%"));
        }

        // Filter by Status
        if (input.Status.HasValue)
        {
            query = query.Where(x => x.Status == input.Status.Value);
        }

        return query;
    }
}
