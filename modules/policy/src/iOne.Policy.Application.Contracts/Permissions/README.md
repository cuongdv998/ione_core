...
Thư mục này chứa các permission definitions cho module Policy.
...
- `GroupName` = `"{ModuleName}{EntityName}"` (ví dụ: `"PolicyPolicyType"`)
- `Default` = `GroupName` (không phải `"{GroupName}.Default"`)
- Các permission names = `"{GroupName}.{Action}"` (ví dụ: `"PolicyPolicyType.Create"`)
