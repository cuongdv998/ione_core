...
Thư mục này chứa các permission definitions cho module File.
...
- `GroupName` = `"{ModuleName}{EntityName}"` (ví dụ: `"FileFileType"`)
- `Default` = `GroupName` (không phải `"{GroupName}.Default"`)
- Các permission names = `"{GroupName}.{Action}"` (ví dụ: `"FileFileType.Create"`)
