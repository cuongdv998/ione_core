...
Thư mục này chứa các permission definitions cho module Report.
...
- `GroupName` = `"{ModuleName}{EntityName}"` (ví dụ: `"ReportReportType"`)
- `Default` = `GroupName` (không phải `"{GroupName}.Default"`)
- Các permission names = `"{GroupName}.{Action}"` (ví dụ: `"ReportReportType.Create"`)
