...
Thư mục này chứa các permission definitions cho module Customer.
...
- `GroupName` = `"{ModuleName}{EntityName}"` (ví dụ: `"CustomerCustomerType"`)
- `Default` = `GroupName` (không phải `"{GroupName}.Default"`)
- Các permission names = `"{GroupName}.{Action}"` (ví dụ: `"CustomerCustomerType.Create"`)
