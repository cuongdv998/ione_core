...
using iOne.ResRisks; // For Entity, Manager, Repository
...
[Authorize(ResRiskPermissions.Default)]
public class ResRiskAppService : CrudAppService<
    ResRisk,
    ResRiskDto,
    Guid,
    GetResRisksInput,
    CreateResRiskDto,
    UpdateResRiskDto>, IResRiskAppService
{
    protected ResRiskManager Manager { get; }
    protected IResRiskRepository RiskRepository { get; }

    public ResRiskAppService(
        IResRiskRepository repository,
        ResRiskManager manager)
        : base(repository)
    {
        Manager = manager;
        RiskRepository = repository;
        LocalizationResource = typeof(MasterResource);
        GetPolicyName = ResRiskPermissions.View;
        GetListPolicyName = ResRiskPermissions.View;
        CreatePolicyName = ResRiskPermissions.Create;
        UpdatePolicyName = ResRiskPermissions.Edit;
        DeletePolicyName = ResRiskPermissions.Delete;
    }

    public override async Task<ResRiskDto> CreateAsync(CreateResRiskDto input)
    {
        // Validate duplicate code
        var normalizedCode = input.Code?.Trim().ToUpperInvariant();
        if (!string.IsNullOrWhiteSpace(normalizedCode))
        {
            if (await RiskRepository.IsCodeExistsAsync(normalizedCode))
            {
                throw new UserFriendlyException(
                    L["ResRisk:CodeExists"].Value.Replace("{Code}", normalizedCode)
                );
            }
        }

        var entity = new ResRisk(
        ...
        return ObjectMapper.Map<ResRisk, ResRiskDto>(entity);
    }

    public override async Task<ResRiskDto> UpdateAsync(Guid id, UpdateResRiskDto input)
    {
    ...
    public override async Task DeleteAsync(Guid id)
    {
        ...
        entity!.UpdateStatus(ResRiskStatus.Deactive);
        await Repository.UpdateAsync(entity);
        await CurrentUnitOfWork.SaveChangesAsync();
    }

    protected override async Task<IQueryable<ResRisk>> CreateFilteredQueryAsync(GetResRisksInput input)
    {
