...
using iOne.ResCarBrands; // For Entity, Manager, Repository
using iOne.ResCarCategories; // For IResCarCategoryRepository
using iOne.ResCarModels; // For IResCarModelRepository
...
[Authorize(ResCarBrandPermissions.Default)]
public class ResCarBrandAppService : CrudAppService<
    ResCarBrand,
    ResCarBrandDto,
    Guid,
    GetResCarBrandsInput,
    CreateResCarBrandDto,
    UpdateResCarBrandDto>, IResCarBrandAppService
{
    protected ResCarBrandManager Manager { get; }
    ...
    public ResCarBrandAppService(
        IResCarBrandRepository repository,
        ResCarBrandManager manager,
        ...
        : base(repository)
    {
        ...
        GetPolicyName = ResCarBrandPermissions.View;
        GetListPolicyName = ResCarBrandPermissions.View;
        CreatePolicyName = ResCarBrandPermissions.Create;
        UpdatePolicyName = ResCarBrandPermissions.Edit;
        DeletePolicyName = ResCarBrandPermissions.Delete;
    }

    public override async Task<ResCarBrandDto> CreateAsync(CreateResCarBrandDto input)
    {
        // Validate duplicate code
        var normalizedCode = input.Code?.Trim().ToUpperInvariant();
        if (!string.IsNullOrWhiteSpace(normalizedCode))
        {
            if (await CarBrandRepository.IsCodeExistsAsync(normalizedCode))
            {
                throw new UserFriendlyException(
                    L["ResCarBrand:CodeExists", normalizedCode].Value
                );
            }
        }

        var entity = new ResCarBrand(
        ...
        return ObjectMapper.Map<ResCarBrand, ResCarBrandDto>(entity);
    }

    public override async Task<ResCarBrandDto> UpdateAsync(Guid id, UpdateResCarBrandDto input)
    {
    ...
    public override async Task DeleteAsync(Guid id)
    {
        ...
        // Check if there are any car models using this car brand
        if (await CarModelRepository.HasActiveModelsForBrandAsync(id))
        {
            throw new UserFriendlyException(L["ResCarBrand:HasCarModels"]);
        }

        // Check if there are any car categories using this car brand
        if (await CarCategoryRepository.HasCategoriesForBrandAsync(id))
        {
            throw new UserFriendlyException(L["ResCarBrand:HasCarCategories"]);
        }
    ...
    protected override async Task<IQueryable<ResCarBrand>> CreateFilteredQueryAsync(GetResCarBrandsInput input)
    {
    ...
    public virtual async Task<ImportResCarBrandResultDto> ImportExcelAsync(byte[] fileBytes)
    {
        var result = new ImportResCarBrandResultDto();
        var errors = new List<ImportResCarBrandErrorDto>();
        ...
        if (worksheet == null)
        {
            throw new UserFriendlyException(L["Master:ResCarBrand:ExcelFileInvalid"]);
        }
        ...
        while (!worksheet.Row(rowNumber).IsEmpty())
        {
            ...
            var rowErrors = new List<ImportResCarBrandErrorDto>();
            ...
            {
                rowErrors.Add(new ImportResCarBrandErrorDto
                {
                    RowNumber = rowNumber,
                    Field = "Code",
                    Message = L["Master:ResCarBrand:CodeRequired"],
                    Value = code
                });
                codeIsValid = false;
            }
            else if (code.Length > 50)
            {
                rowErrors.Add(new ImportResCarBrandErrorDto
                {
                    RowNumber = rowNumber,
                    Field = "Code",
                    Message = L["Master:ResCarBrand:CodeMaxLength"],
                    Value = code
                });
                codeIsValid = false;
            }
            else if (!Regex.IsMatch(code, @"^[A-Z0-9_]+$"))
            {
                rowErrors.Add(new ImportResCarBrandErrorDto
                {
                    RowNumber = rowNumber,
                    Field = "Code",
                    Message = L["Master:ResCarBrand:CodeInvalid"],
                    Value = code
                });
                codeIsValid = false;
            }
            ...
            if (codeIsValid && !string.IsNullOrWhiteSpace(code))
            {
                ...
                {
                    var message = L["Master:ResCarBrand:CodeDuplicateInFile"].Value.Replace("{0}", code);
                    rowErrors.Add(new ImportResCarBrandErrorDto
                    {
                        RowNumber = rowNumber,
                        Field = "Code",
                ...
                else
                {
                    ...
                    if (await CarBrandRepository.IsCodeExistsAsync(code))
                    {
                        var message = L["Master:ResCarBrand:CodeExists"].Value.Replace("{Code}", code);
                        rowErrors.Add(new ImportResCarBrandErrorDto
                        {
                            RowNumber = rowNumber,
                            Field = "Code",
            ...
            if (string.IsNullOrWhiteSpace(name))
            {
                rowErrors.Add(new ImportResCarBrandErrorDto
                {
                    RowNumber = rowNumber,
                    Field = "Name",
                    Message = L["Master:ResCarBrand:NameRequired"],
                    Value = name
                });
            }
            else if (name.Length > 250)
            {
                rowErrors.Add(new ImportResCarBrandErrorDto
                {
                    RowNumber = rowNumber,
                    Field = "Name",
                    Message = L["Master:ResCarBrand:NameMaxLength"],
                    Value = name
                });
            }
            ...
            if (description != null && description.Length > 500)
            {
                rowErrors.Add(new ImportResCarBrandErrorDto
                {
                    RowNumber = rowNumber,
                    Field = "Description",
                    Message = L["Master:ResCarBrand:DescriptionMaxLength"],
                    Value = description
                });
            }
            ...
            if (rowErrors.Count == 0 && !string.IsNullOrWhiteSpace(code) && !string.IsNullOrWhiteSpace(name) && statusIsValid)
            {
                try
                {
                    var entity = new ResCarBrand(
                ...
                catch (BusinessException ex)
                {
                    ...
                    if (message.Contains("Master:ResCarBrand:") || message.Contains("Master::ResCarBrand:"))
                    {
    ...
    public virtual Task<byte[]> ExportTemplateAsync()
    {
        ...
        var worksheet = workbook.Worksheets.Add("Hãng xe");
        ...
        worksheet.Cell(2, 2).Value = "Toyota Motor Corporation";
