...
Thư mục này chứa các permission definitions cho module Claim.
...
- `GroupName` = `"{ModuleName}{EntityName}"` (ví dụ: `"ClaimClaimType"`)
- `Default` = `GroupName` (không phải `"{GroupName}.Default"`)
- Các permission names = `"{GroupName}.{Action}"` (ví dụ: `"ClaimClaimType.Create"`)
