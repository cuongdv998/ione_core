...
Thư mục này chứa các permission definitions cho module AppMobile.
...
- `GroupName` = `"{ModuleName}{EntityName}"` (ví dụ: `"AppMobileAppMobileType"`)
- `Default` = `GroupName` (không phải `"{GroupName}.Default"`)
- Các permission names = `"{GroupName}.{Action}"` (ví dụ: `"AppMobileAppMobileType.Create"`)
