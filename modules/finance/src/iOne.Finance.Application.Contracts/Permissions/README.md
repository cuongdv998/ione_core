...
Thư mục này chứa các permission definitions cho module Finance.
...
- `GroupName` = `"{ModuleName}{EntityName}"` (ví dụ: `"FinanceFinanceType"`)
- `Default` = `GroupName` (không phải `"{GroupName}.Default"`)
- Các permission names = `"{GroupName}.{Action}"` (ví dụ: `"FinanceFinanceType.Create"`)
