...
public class MongoAuditLogRepository : MongoDbRepository<IAuditLoggingMongoDbContext, AuditLog, Guid>, IAuditLogRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        var query = await GetListQueryAsync(
            ...
            clientIpAddress,
            correlationId,
            maxDuration, // Map maxDuration to maxExecutionDuration
            minDuration, // Map minDuration to minExecutionDuration
            hasException,
            httpStatusCode,
            includeDetails,
            cancellationToken
    ...
    public virtual async Task<(List<AuditLog> Items, long TotalCount)> GetListWithCountAsync(
        string sorting = null,
        int maxResultCount = 50,
        int skipCount = 0,
        DateTime? startTime = null,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Execute count and list queries in parallel using the same query base
        var countTask = query.LongCountAsync(GetCancellationToken(cancellationToken));
        var listTask = query
            .OrderBy(sorting.IsNullOrWhiteSpace() ? (nameof(AuditLog.ExecutionTime) + " DESC") : sorting)
            .PageBy(skipCount, maxResultCount)
            .ToListAsync(GetCancellationToken(cancellationToken));
        ...
        return (await listTask, await countTask);
    }
    ...
    protected virtual async Task<IQueryable<AuditLog>> GetListQueryAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // Support both maxDuration/minDuration (for backward compatibility) and maxExecutionDuration/minExecutionDuration
        var maxDuration = maxExecutionDuration;
        var minDuration = minExecutionDuration;
        ...
        return (await GetQueryableAsync(cancellationToken))
            ...
            .WhereIf(maxDuration != null && maxDuration.Value > 0, auditLog => auditLog.ExecutionDuration <= maxDuration)
            .WhereIf(minDuration != null && minDuration.Value > 0, auditLog => auditLog.ExecutionDuration >= minDuration);
    }
    ...
    public virtual async Task<List<EntityChange>> GetEntityChangeListAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = await GetEntityChangeListQueryAsync(auditLogId, startTime, endTime, changeType, entityId, entityTypeFullName, includeDetails, cancellationToken);
    ...
    public virtual async Task<long> GetEntityChangeCountAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = await GetEntityChangeListQueryAsync(auditLogId, startTime, endTime, changeType, entityId, entityTypeFullName, includeDetails: false, cancellationToken);
    ...
    public virtual async Task<(List<EntityChange> Items, long TotalCount)> GetEntityChangeListWithCountAsync(
        string sorting = null,
        int maxResultCount = 50,
        int skipCount = 0,
        Guid? auditLogId = null,
