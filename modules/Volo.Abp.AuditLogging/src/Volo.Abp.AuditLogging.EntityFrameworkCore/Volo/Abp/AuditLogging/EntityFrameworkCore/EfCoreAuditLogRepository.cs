...
public class EfCoreAuditLogRepository : EfCoreRepository<IAuditLoggingDbContext, AuditLog, Guid>, IAuditLogRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Fix: Execute count and list queries sequentially to avoid DbContext concurrency issues
        // EF Core doesn't allow concurrent operations on the same DbContext instance
        var totalCount = await query.LongCountAsync(GetCancellationToken(cancellationToken));
    ...
    public virtual async Task<long> GetEntityChangeCountAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = await GetEntityChangeListQueryAsync(auditLogId, startTime, endTime, changeType, entityId, entityTypeFullName, includeDetails: false);
