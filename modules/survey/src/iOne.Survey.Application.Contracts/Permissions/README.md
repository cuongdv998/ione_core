...
Thư mục này chứa các permission definitions cho module Survey.
...
- `GroupName` = `"{ModuleName}{EntityName}"` (ví dụ: `"SurveySurveyType"`)
- `Default` = `GroupName` (không phải `"{GroupName}.Default"`)
- Các permission names = `"{GroupName}.{Action}"` (ví dụ: `"SurveySurveyType.Create"`)
