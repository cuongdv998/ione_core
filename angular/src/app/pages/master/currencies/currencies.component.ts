...
import { ResCurrencyDto, CreateResCurrencyDto, UpdateResCurrencyDto, GetResCurrenciesInput } from '@/proxy/master/res-currencies/models';
import { ResCurrencyStatus } from '@/proxy/res-currencies/res-currency-status.enum';
import { CurrencySearchForm, CurrencyFormData } from './currencies.models';
...
export class ResCurrencyComponent implements OnInit {
  // Permissions
  readonly PERMISSIONS = {
    CREATE: 'MasterResCurrency.Create',
    UPDATE: 'MasterResCurrency.Edit',
    DELETE: 'MasterResCurrency.Delete',
    VIEW: 'MasterResCurrency.View'
  };

  // Data
  currencies: ResCurrencyDto[] = [];
  totalCount = 0;
  loading = true;
  pageSize = 10;
  ...
  formData: CurrencyFormData = this.getEmptyForm();
  selectedCurrency?: ResCurrencyDto;

  // Search form
  searchForm: CurrencySearchForm = {
    code: null,
    name: null,
    status: null
  };
  ...
  // Status options
  statusOptions: Array<{ label: string; value: ResCurrencyStatus }> = [];

  // Table
  columns: TableColumn[] = [];
  actions: TableAction<ResCurrencyDto>[] = [];
  ...
  private initializeStatusOptions(): void {
    this.statusOptions = [
      { label: this.localizationService.localize('Master::ResCurrency:Active'), value: ResCurrencyStatus.Active },
      { label: this.localizationService.localize('Master::ResCurrency:Deactive'), value: ResCurrencyStatus.Deactive }
    ];
  }
  ...
  private initializeColumns(): void {
    this.columns = [
      {
        field: 'code',
        header: this.localizationService.localize('Master::ResCurrency:Code'),
        sortable: true,
        width: '150px'
      },
      {
        field: 'name',
        header: this.localizationService.localize('Master::ResCurrency:Name'),
        sortable: true,
        width: '250px'
      },
      {
        field: 'description',
        header: this.localizationService.localize('Master::ResCurrency:Description'),
        sortable: false,
        width: '300px'
      },
      {
      ...
      {
        field: 'status',
        header: this.localizationService.localize('Master::ResCurrency:Status'),
        sortable: true,
        type: 'text',
        width: '120px',
        freeze: 'right',
        ...
        cellClass: (value: any) => {
          const statusValue = typeof value === 'number' ? value : 
            (value === ResCurrencyStatus.Active ? 0 : 1);
  ...
  /**
   * Load currencies with lazy loading
   */
  loadData(event: TableLazyLoadEvent): void {
    this.currentLazyLoadEvent = event;
    this.loading = true;

    const input: GetResCurrenciesInput = {
    ...
    this.currencyService.getList(input).subscribe({
      next: (result) => {
   ...
   */
  openEditDialog(currency: ResCurrencyDto): void {
    this.dialogMode = 'edit';
    this.selectedCurrency = currency;
    this.formData = {
      code: currency.code || '', // Readonly trong edit mode
      name: currency.name || '',
      description: currency.description || '',
      status: currency.status ?? ResCurrencyStatus.Active
    };
    this.dialogVisible = true;
  }
  ...
  private validateForm(): boolean {
    if (this.dialogMode === 'create') {
      ...
      if (!this.formData.code || this.formData.code.trim() === '') {
        this.messageService.add({
          severity: 'error',
          summary: this.localizationService.localize('Master::Error'),
          detail: this.localizationService.localize('Master::ResCurrency:CodeRequired')
        });
        return false;
      }
      ...
      if (this.formData.code.length > 50) {
        this.messageService.add({
          severity: 'error',
          summary: this.localizationService.localize('Master::Error'),
          detail: this.localizationService.localize('Master::ResCurrency:CodeMaxLength')
        });
        return false;
      }
      ...
      if (!this.validateCode(this.formData.code)) {
        this.messageService.add({
          severity: 'error',
          summary: this.localizationService.localize('Master::Error'),
          detail: this.localizationService.localize('Master::ResCurrency:CodeInvalid')
        });
        return false;
      }
    }
    ...
    if (!this.formData.name || this.formData.name.trim() === '') {
      this.messageService.add({
        severity: 'error',
        summary: this.localizationService.localize('Master::Error'),
        detail: this.localizationService.localize('Master::ResCurrency:NameRequired')
      });
      return false;
    }
    ...
    if (this.formData.name.length > 250) {
      this.messageService.add({
        severity: 'error',
        summary: this.localizationService.localize('Master::Error'),
        detail: this.localizationService.localize('Master::ResCurrency:NameMaxLength')
      });
      return false;
    }
    ...
    if (this.formData.description && this.formData.description.length > 500) {
      this.messageService.add({
        severity: 'error',
        summary: this.localizationService.localize('Master::Error'),
        detail: this.localizationService.localize('Master::ResCurrency:DescriptionMaxLength')
      });
      return false;
    }
  ...
  translateErrorMessage(message: string, error?: any): string {
    ...
    // e.g., "Master:ResCurrency:CodeExists" -> "Master::ResCurrency:CodeExists"
  ...
  private create(): void {
    ...
    const createDto: CreateResCurrencyDto = {
    ...
    this.currencyService.create(createDto).subscribe({
      next: () => {
        this.messageService.add({
          severity: 'success',
          summary: this.localizationService.localize('Master::Success'),
          detail: this.localizationService.localize('Master::ResCurrency:CreatedSuccessfully')
        });
  ...
  private update(): void {
    ...
    const updateDto: UpdateResCurrencyDto = {
      name: this.formData.name.trim(),
      description: this.formData.description?.trim() || undefined,
      status: this.formData.status
    };

    this.currencyService.update(this.selectedCurrency.id, updateDto).subscribe({
      next: () => {
        this.messageService.add({
          severity: 'success',
          summary: this.localizationService.localize('Master::Success'),
          detail: this.localizationService.localize('Master::ResCurrency:UpdatedSuccessfully')
        });
        this.dialogVisible = false;
        this.search();
      },
   ...
   */
  delete(currency: ResCurrencyDto): void {
    if (!currency.id) {
      return;
    }

    this.confirmationService.confirm({
      message: this.localizationService.localize('Master::ResCurrency:DeleteConfirm'),
      header: this.localizationService.localize('Master::ResCurrency:Delete'),
        ...
        this.currencyService.delete(currency.id!).subscribe({
          next: () => {
            this.messageService.add({
              severity: 'success',
              summary: this.localizationService.localize('Master::Success'),
              detail: this.localizationService.localize('Master::ResCurrency:DeletedSuccessfully')
            });
            this.search();
          },
          error: (error) => {
   ...
   * Get empty form data
   */
  private getEmptyForm(): CurrencyFormData {
    return {
      code: '',
      name: '',
      description: '',
      status: ResCurrencyStatus.Active
    };
  }
   ...
   */
  formatStatus(status: ResCurrencyStatus | number | string | undefined | null): string {
    ...
    if (typeof status === 'number') {
      ...
      statusValue = status === ResCurrencyStatus.Active ? 0 : 1;
    }
    
    return statusValue === 0
      ? this.localizationService.localize('Master::ResCurrency:Active')
      : this.localizationService.localize('Master::ResCurrency:Deactive');
  }
}
