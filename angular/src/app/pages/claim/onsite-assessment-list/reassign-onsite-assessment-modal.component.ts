...
import type { ReassignOnsiteAssessmentInput } from './onsite-assessment-list.models';
import { OnsiteAssessmentTaskService } from './onsite-assessment-task.service';

@Component({
  selector: 'app-reassign-onsite-assessment-modal',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './reassign-onsite-assessment-modal.component.html',
  styleUrl: './reassign-onsite-assessment-modal.component.scss',
  providers: [MessageService, ConfirmationService],
})
export class ReassignOnsiteAssessmentModalComponent implements OnChanges {
  @Input() visible = false;
  ...
  constructor(
    private readonly onsiteAssessmentTaskService: OnsiteAssessmentTaskService,
  ...
  submit(): void {
    if (!this.workTaskId || !this.assigneeOrganizationId || !this.assigneeId || !this.startDate || !this.endDate) {
      this.messageService.add({
        severity: 'warn',
        summary: this.localizationService.localize('AbpUi::ValidationError'),
        detail: 'Vui lòng nhập đầy đủ thông tin giao lại giám định hiện trường.',
      });
      return;
    }
    ...
    const input: ReassignOnsiteAssessmentInput = {
    ...
    this.saving = true;
    this.onsiteAssessmentTaskService.reassign(this.workTaskId, input).subscribe({
      next: () => {
        this.messageService.add({
          severity: 'success',
          summary: this.localizationService.localize('AbpUi::Success'),
          detail: 'Giao lại giám định hiện trường thành công.',
        });
        this.saving = false;
